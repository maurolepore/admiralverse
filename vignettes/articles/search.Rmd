...
but it can also be small, company-specific or personal, and not on CRAN.
...
admiralverse, and a URL template pointing to each topic in each package.
...
Before creating a single documentation table, consider the different kinds.

```{r kinds}
online |> count(type, keyword)
```

Here it makes sense to group vignettes, datasets, and public functions.
...
If this sounds useful you may explore the source code of the toy
[admiralverse](https://maurolepore.github.io/admiralverse) meta-package, which
models the main use case of [dverse](https://maurolepore.github.io/dverse/), or
